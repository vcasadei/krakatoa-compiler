...
public class Comp {
	...
	private void printReport(int numSourceFiles, PrintWriter report) {
		...
		if (numSourceFiles - numSourceFilesWithErros != 0) {
			report.println(
					this.wereButShouldNotList.size() + " erros foram sinalizados em "
							+ (numSourceFiles - numSourceFilesWithErros) + " arquivos sem erro (" + (int) (100.0
									* this.wereButShouldNotList.size() / (numSourceFiles - numSourceFilesWithErros))
					+ "%)");
		}
		...
		if (correctList.size() > 0) {
			report.println("######################################################");
			report.print(
					"Em todos os testes abaixo, o compilador sinalizou o erro na linha correta (quando o teste tinha erros) ");
			report.print("ou não sinalizou o erro (quando o teste NÃO tinha erros). Mas é necessário conferir se as ");
			report.print(
					"mensagens emitidas pelo compilador são compatíveis com as mensagens de erro sugeridas pelas chamadas aos ");
	...
	/**
	 * number of tests with erros. That is, the number of tests in which there
	 * is a call to metaobject {@literal @}ce.
	 */
		...
		for (MetaobjectCall moCall : program.getMetaobjectCallList()) {
			if (moCall.getName().equals("ce")) {
				...
				if (!program.hasCompilationErrors()) {
					...
					this.shouldButWereNotList.add(filename + ", " + lineNumber + ", " + message + whatToCorrect);
					if (foundCE)
						outError.println(
					...
					if (lineNumber != lineOfError) {
						...
						this.wereButWrongLineList.add(filename + "\n" + "    correto:    " + lineNumber + ", "
								+ ceMessage + " " + whatToCorrect + "\n" + "    sinalizado: " + lineOfError + ", "
								+ compilerMessage);
					} else {
